...
// Servo declarations
#define SERVO1_PIN 8  // servo signal pin
#define SERVO2_PIN 9  // servo signal pin
Servo SERVO1;
Servo SERVO2;
...
// report tranmission frequency
unsigned long last_send = 0;
unsigned long send_delay = 10;
  ...
  if(Serial1.available()>= 1)
  {
    ...
    if (enable_comms)
    {
      ...
      if (message_identifier == signal) 
      {
        ...
        Serial.print("wait_report_response = ");
        Serial.println(wait_report_response);
      }
      */
      if(message_identifier ==  write_msg_prefix) // Instruction prefix flag
      {
        ...
        Serial.println(mot_A_val1);
        mot_A_val2 = Serial1.read();
        Serial.println(mot_A_val2);
        mot_B_val1 = Serial1.read();
        Serial.println(mot_B_val1);
        mot_B_val2 = Serial1.read();
        Serial.println(mot_B_val2);
        write_motor_vals();
      ...
      else if (message_identifier == '-') // wifi disconnected, stop motors
      {
        ...
        write_motor_vals();
        enable_comms = false;
        Serial.println("TCP Disconnected: Turning off motors");
      }
      else if (message_identifier == '+') 
      {
        Serial.println("TCP connected");
        //Serial.println("Sending handshake to notify PC");
        //Serial1.write(signal); // wifi connected, establish handshake  
      } 
    }
    else if(enable_comms == false) // comms are disabled in order to avoid noise at mcu node startup
    {
      Serial.println("recieving data while comms status 0");
...
{
  if(millis() > (long)(last_send + send_delay))
  {
    //Serial.println("Measuring distance and reading wheel speed");
    // measure distance
    measure_distance(); 
    //Serial.println("Preparing report message");
    // report message example byte array = B1:R B2:W B3:M1IN1 B4:M1IN2 B5:M2IN1 B6:M2IN2
    // write prefixes
    //Serial.println(report_msg_prefix);
    Serial1.write(report_msg_prefix); // "report" message flag
    //Serial.println(write_msg_prefix);
